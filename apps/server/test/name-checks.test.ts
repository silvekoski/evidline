...
it("reports the primary name with its reason and each reviewer with its reason", () => {
  ...
  expect(report.primary).toMatchObject({ egressId: "eg-1", model: "mistralai/Large", name: "Reactor pressure", reason: "Leads the flow." });
...
it("reports no primary name when the naming call is off", () => {
  ...
  expect(nameCheckReport(f.ctx, head)).toEqual({ pending: null, primary: null, checks: [] });
});
