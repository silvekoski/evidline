...
export function egressTotals(db: Db, runId?: string): EgressTotals {
  ...
  return {
    ...
    sentBytes: records.filter((r) => r.status === "sent" || r.status === "error").reduce((sum, r) => sum + r.payloadBytes, 0),
    calls: records.length,
    sent: count("sent"),
    blocked: count("blocked"),
    off: count("off"),
    scannerHits: records.filter((r) => r.guards.some((g) => (g.name === "leak" || g.name === "names") && !g.pass)).length,
    hosts: [...new Set(records.filter((r) => r.status === "sent").flatMap((r) => (r.provider ? [r.provider.host] : [])))],
  };
}
